...
def processdate(datatype,yymm):
    ...
    for rec in resultrec:
        ...
        try:
            ...
            if ctr > 500:
                ...
                os.system('/opt/vertica/bin/vsql -f '+vertflsql+'.run -U dbadmin -w c@llhome -h callhomelab-vertica01 callhomedb  > /root/proc/log/'+datatype+'_'+str(yymm)+'.sql.log');
    ...
    os.system('mv '+vertflsql+' '+vertflsql+'.run');
    os.system('/opt/vertica/bin/vsql -f '+vertflsql+'.run -U dbadmin -w c@llhome -h callhomelab-vertica01 callhomedb  > /root/proc/log/'+datatype+'_'+str(yymm)+'.sql.log');
    os.system('mv '+vertflsql+' '+vertflsql+'.bak');
    currtime=time.strftime('%Y/%m/%d %H:%M:%S');
    hwlog.write(currtime+':'+'Done loading to vertica\n');
    resultrec.close();
    currtime=time.strftime('%Y/%m/%d %H:%M:%S');
