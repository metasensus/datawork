...
def get_data():
    try:
        ...
        optimuslogger=open('log/optimus_json.log','a');
        optimuslogger.write('---------------------------------------------------------------------------------------------------------------------------------------------\n');
        sqlstmt='SELECT QUERY_ID,DB_SOURCE,DB_USER,DB_TYPE,HOSTNAME,QUERY_STRING,REPORT_TITLE,OUTPUT_FILE_NAME,FILE_LOCATION FROM GRAPH_REPORT_DATA where enabled=1 and JSON_TEMPLATE=\'graphjson.py\' and AGG_UPDATE> nvl(OUTPUT_FILE_UPDATE,trunc(sysdate)) order by QUERY_ID';
