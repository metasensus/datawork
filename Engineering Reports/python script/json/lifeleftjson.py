...
def read_data(sourcedb,dbuser,dbtype,hostname,query_string,report_title,output_file,file_location,axis,title,column,xtitle,xcolumn,ytitle,ycolumn):
    try:
        ...
        color=["#3300FF","#FF6600","#FFFF00","#1fcaff","#d9b2ff","#b2b2ff","#0fbff2","#a0a1e5","#0000ff","#c3c6c0","#ffa500","#851f2f","#ff6666","#b30303","#d4d0c5","#062833","#DC143C","#9932CC","#1E90FF","#FF00FF","#66CDAA","#808000"]
        for col in column:
        ...
        for rec in resultset:
            ...
            if(j==len(resultset)):
                datstr+='\n{"'+column[0]+'":"'+str(rec[0])+'\",\"'+column[1]+'":"'+str(rec[1])+'\",\"'+column[2]+'":"'+str(rec[2])+'\",\"'+column[3]+'":"'+str(color[k])+'\"'+'}';
                temp=rec[0];   
        ...
        timestr=time.strftime('%m/%d/%Y %H:%M:%S')+' '+function;
        errlog.write(timestr+'\t Error in the report '+output_file+'\n');
        errlog.write(timestr+'\t Error reported: '+str(sys.exc_info()[1])+ '\n');
...
def get_data():
    try:
        ...
        #optimuslogger=open('log/optimus_json.log','a');
       # optimuslogger.write('---------------------------------------------------------------------------------------------------------------------------------------------\n');
        sqlstmt='SELECT QUERY_ID,DB_SOURCE,DB_USER,DB_TYPE,HOSTNAME,QUERY_STRING,REPORT_TITLE,OUTPUT_FILE_NAME,FILE_LOCATION FROM GRAPH_REPORT_DATA where enabled=1 and JSON_TEMPLATE=\'lifeleftjson.py\'and AGG_UPDATE> nvl(OUTPUT_FILE_UPDATE,trunc(sysdate)) order by QUERY_ID';
