...
def read_data(sourcedb,dbuser,dbtype,hostname,query_string,report_title,output_file,file_location,axis,title,column,xtitle,xcolumn,ytitle,ycolumn,ztitle,zcolumn):
    try:
        ...
        for rec in resultset:
            ...
            if(j==len(resultset)):
                datstr+='\n{"'+column[0]+'":"'+str(rec[0])+'\",\"'+column[1]+'":"'+str(rec[2])+'\",\"'+column[2]+'":"'+str(rec[1])+'\"'+'}';
                temp=rec[0];   
...
def get_data():
    try:
        ...
        optimuslogger=open('log/optimus_json.log','a');
        optimuslogger.write('---------------------------------------------------------------------------------------------------------------------------------------------\n');
        sqlstmt='SELECT QUERY_ID,DB_SOURCE,DB_USER,DB_TYPE,HOSTNAME,QUERY_STRING,REPORT_TITLE,OUTPUT_FILE_NAME,FILE_LOCATION FROM GRAPH_REPORT_DATA where enabled=1 and JSON_TEMPLATE=\'informjson.py\'and AGG_UPDATE> nvl(OUTPUT_FILE_UPDATE,trunc(sysdate)) order by QUERY_ID';
