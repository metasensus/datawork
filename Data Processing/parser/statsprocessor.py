...
def processFile(STATS_SPLITFILE_ID, STATS_SPLITFILE_NAME, STATS_SPLITFILE_PATH, STATS_SPLITFILE_TYPE ,INSERVSERIAL ,DATAFOLDER ,MACHINEID ):
    try:
        ...
        if versionExist(STATS_SPLITFILE_TYPE) == 1:
            ...
            if string.find(STATS_SPLITFILE_TYPE,'status') >=0  and len(version)==0:
                version='2.3.1'
        else:
            version='2.3.1';
...
def procExec():
    try:
        ...
        sqlstmt='select machineid,numberofthreads,processloc,number_of_files_per_run,delay_seconds,priority from statsprocessingmachine where ipaddress=\''+ipadd+'\' and enable =1';
        ...
        if numthreadstocopy > 0:
            ...
            while totrows <= numthreadstocopy:
                sqlstmt='select STATS_FILEID,STATS_SPLITFILE_ID,STATS_SPLITFILE_NAME,STATS_SPLITFILE_PATH,STATSID,STATS_SPLIT_FILE_TYPE from STATS_SPLIT_FILES'+tabname;
                ...
                for flrec in FileRec:
                    ...
                    sqlst='begin Update STATS_SPLIT_FILES'+tabname+' set FILE_PROCESS_STATUS=2 where STATS_SPLITFILE_ID='+str(flrec[1])+'; commit; end;';
